...
from .malicious_utils import MNIST_label_flipping

class TrainerMNISTSketch:
    def __init__(self,num_id, mode) -> None:
        ...
        self.metrics_names = ["accuracy", "activation_layer"]
    ...
    def split_data(self):
        ...
        train_dataset = datasets.MNIST(root="dataset/", download=True, train=True, transform=transforms.Compose([
        ...
        if(self.num_id == 0):
            MNIST_label_flipping(train_dataset)
        

        train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)

        test_dataset = datasets.MNIST(root="dataset/", download=True, train=False, transform=transforms.Compose([
                                                                                            transforms.Resize((32,32)),
                                                                                            transforms.ToTensor(),
                                                                                            transforms.Normalize(mean=(0.1307,), std=(0.3081,))]))
    ...
    def all_metrics(self):
        ...
        values = [self.eval_model(), self.weights['fc2.bias'].tolist()]
        dict_metrics = dict(zip(metrics_names, values))
        return dict_metrics
