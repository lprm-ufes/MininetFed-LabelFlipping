...
class TrainerMNIST:
    ...
    def all_metrics(self):
        ultima_camada = self.model.layers[-1]
        pesos = ultima_camada.get_weights()

        metrics_names = self.model.metrics_names
        metrics_names.append("activation_layer")
        values = [self.model.evaluate(x=self.x_test, y=self.y_test, verbose=False), pesos[1].tolist()]
